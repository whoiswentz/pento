defmodule Pento.Game.Board do
  alias Pento.Game.{Pentomino, Shape, Point}

  @attrs [:active_pento, :palette, :completed_pentos, :points]
  @enforce_keys @attrs -- [:active_pento, :completed_pentos]

  @type t :: %{
          active_pento: Pentomino.t(),
          completed_pentos: Pentomino.t(),
          palette: atom(),
          points: list(Point.t())
        }
  ...
  defp rect(x, y) do
    for x <- 1..x, y <- 1..y, do: Point.new(x, y)
  end
  ...
  defp new_pento(board, shape_name) do
    Pentomino.new(shape_name, 0, false, midpoints(board))
  end
  ...
  defp midpoint(i), do: round(Enum.max(i) / 20)
